...
export default function SelectedProject({ session }: Props) {
    const { data: project, error } = useSWR<ReadablePersonalProjectData[]>('/api/projects', fetcher);
    ...
    return (
        <>
            ...
            {isOpen && (
                <div
                    ref={ref}
                    className={clx('bg-black/90 border border-zinc-700 absolute top-[72px] left-[120px] z-[100] text-base list-none rounded-xl', isOpen ? 'block' : 'hidden')}
                >
                    <div className={'p-4'}>

...
export default async function handler(req: NextApiRequest, res: NextApiResponse<RestRequest<any>>) {
    ...
    const errors = new RestHelper(req, res).checkIsVariableIsDefined(projectId, 'Project Id').checkIsVariableIsDefined(email, 'New Owner User Id').checkEmail(email).checkErrors();
    if (errors) return;

    try {
        const data = await new ProjectRepository(prisma.project).transferOwnership(projectId, userId, email);

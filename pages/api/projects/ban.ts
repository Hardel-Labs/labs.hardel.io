...
export default async function handler(req: NextApiRequest, res: NextApiResponse<RestRequest<any>>) {
    ...
    const errors = new RestHelper(req, res).checkIsVariableIsDefined(projectId, 'Project ID').checkIsVariableIsDefined(removedUserId, 'Removed user ID').checkErrors();
    if (errors) return;

    try {
        const data = await new ProjectRepository(prisma.project).removeUserInProject(projectId, userId, removedUserId);
